...
static AFX_EXTENSION_MODULE KBHookDLLDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("KBHookDLL.DLL 初期化中\n");

		// 拡張 DLL を１回だけ初期化します。
		if (!AfxInitExtensionModule(KBHookDLLDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(KBHookDLLDLL);

		g_hDllInst = hInstance;
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("KBHookDLL.DLL 終了中\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(KBHookDLLDLL);
	}
	return 1;   // OK
}

HINSTANCE GetThisHInst(void)
{
	return g_hDllInst;
}
